...
export default class Player {
    ...
    _getMTV(x, y) {
        ...
        return getCollisionMTVForMovement(
            getPlayerPoints(x, y, size),
            x,
            y,
            x,
            y,
    ...
    tick(dt) {
        ...
        for (let s = 0; s < SUBSTEPS; s++) {
            ...
            const targetSpeed =
                frameInputDir *
                (this.onGround ? PlayerConstants.speed : PlayerConstants.speed * 0.9);
            const accel = (this.onGround ? 120 : 60) * timeFactor;
            const decel = (this.onGround ? 0.6 : 0.1) * timeFactor;

            if (frameInputDir !== 0) {
                if (this.velX < targetSpeed)
                    this.velX = Math.min(this.velX + accel, targetSpeed);
                else if (this.velX > targetSpeed)
                    this.velX = Math.max(this.velX - accel, targetSpeed);
            } else {
            ...
            if (this.velY > PlayerConstants.maxFallSpeed)
                this.velY = PlayerConstants.maxFallSpeed;
    ...
    drawAt(x, y) {
        ...
        this.ctx.fillRect(
            x,
            y + Math.min((this.velY / 3), 2),
            PlayerConstants.getPlayerSize(),
            PlayerConstants.getPlayerSize() + Math.min((this.velY / 3), 5),
        );
        this.ctx.restore();
    }
}
